...
private:
  ...
  const edm::EDGetTokenT<std::vector<Trajectory>> l3TrajCollectionToken_;
...
Phase2HLTMuonSelectorForL3::Phase2HLTMuonSelectorForL3(const edm::ParameterSet& iConfig)
    : l1TkMuCollToken_(consumes<l1t::TrackerMuonCollection>(iConfig.getParameter<edm::InputTag>("l1TkMuons"))),
      ...
      l3TrajCollectionToken_(consumes<std::vector<Trajectory>>(iConfig.getParameter<edm::InputTag>("l3Tracks"))),
      ...
      copyTrajectories_(iConfig.getParameter<bool>("copyTrajectories")){
  if (IOFirst_) {
...
void Phase2HLTMuonSelectorForL3::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  auto const& l3trajIn = copyTrajectories_ ? iEvent.get(l3TrajCollectionToken_) : dummy;
  ...
  if (IOFirst_) {
    ...
    for (const size_t index : goodL3Indexes) {
      ...
      if (copyTrajectories_) {
        L3OITracksFilteredTrajectories->push_back(l3trajIn[index]);
        assert(L3OITracksFilteredTrajectories->back().measurements().size() == L3OITracksFiltered->back().recHitsSize());
        //L3OITracksFilteredTrajectoryTrackAssociations->insert(edm::Ref<std::vector<Trajectory> >(trajRefProd, L3OITracksFilteredTrajectories->size() - 1),
        //                                                      reco::TrackRef(rTracks, L3OITracksFiltered->size() - 1));
      }
    }
    ...
    edm::OrphanHandle<reco::TrackCollection> theOITracks = iEvent.put(std::move(L3OITracksFiltered), "L3OITracksFiltered");
    
    if (copyTrajectories_) {
      auto tsize = L3OITracksFiltered->size();
      assert(L3OITracksFilteredTrajectories->size() == tsize);
      assert(L3OITracksFilteredTrajectoryTrackAssociations->size() == tsize);
      iEvent.put(std::move(L3OITracksFilteredTrajectories));
      //iEvent.put(std::move(L3OITracksFilteredTrajectoryTrackAssociations));

      edm::OrphanHandle<vector<Trajectory>> theOITrajectories = iEvent.put(std::move(L3OITracksFilteredTrajectories));
      for (unsigned index = 0; index < tsize; ++index) {
        edm::Ref<vector<Trajectory>> trajRef(theOITrajectories, index);
        edm::Ref<reco::TrackCollection> tkRef(theOITracks, index);
