...
namespace {
  ...
  public:
    ...
    inline uint32_t addSegment(float tPath, float tMom2, float sigmaMom) {
      ...
      LogTrace("MuonExtenderWithMTD") << "addSegment # " << nSegment_ << " s = " << tPath
    ...
    inline float computeTof(float mass_inv2) const {
      ...
      for (uint32_t iSeg = 0; iSeg < nSegment_; iSeg++) {
        ...
        LogTrace("MuonExtenderWithMTD") << " TOF Segment # " << iSeg + 1 << " p = " << std::sqrt(segmentMom2_[iSeg])
                                         << " tof = " << tof;
        ...
        LogTrace("MuonExtenderWithMTD") << "TOF Segment # " << iSeg + 1 << std::fixed << std::setw(6)
    ...
    inline std::pair<float, float> getSegmentPathAndMom2(uint32_t iSegment) const {
      if (iSegment >= nSegment_) {
        throw cms::Exception("MuonExtenderWithMTD") << "Requesting non existing track segment #" << iSegment;
      }
  ...
  const TrackTofPidInfo computeTrackTofPidInfo(float magp2,
    ...
    // assume by default the pi hypothesis unless for muon tracks
    if (doMuon)
      tofpid.dt = tofpid.tmtd - tofpid.dt_mu - t_vtx;
    else
      tofpid.dt = tofpid.tmtd - tofpid.dt_pi - t_vtx;

    if (doMuon)
      std::cout << "DeltaT for muon = " << tofpid.dt << std::endl;
    
    tofpid.dterror2 = tofpid.tmtderror * tofpid.tmtderror + t_vtx_err * t_vtx_err;
    tofpid.betaerror = 0.f;
    
    if (addPIDError && !doMuon) {
      ...
      tofpid.dterror2 = tofpid.dterror2 + tofpid.sigma_dt_mu * tofpid.sigma_dt_mu;
    } else {
  ...
  bool trackPathLength(const Trajectory& traj,
    ...
    return trackPathLength(traj, tscbl, thePropagator, pathlength, trs, doMuon);
  }

}  // namespace

template <class TrackCollection>
class MuonExtenderWithMTDT : public TrackExtenderWithMTDT {
public:
  ...
  void produce(edm::Event& ev, const edm::EventSetup& es) final override;
  ...
  reco::Track buildTrack(const reco::TrackRef&,
			 ...
			 float& tofmu,
                         ...
                         float& sigmatofk,
                         float& sigmatofp,
			 float& sigmatofmu) const override;
...
template <class TrackCollection>
MuonExtenderWithMTDT<TrackCollection>::MuonExtenderWithMTDT(const ParameterSet& iConfig)
...
template <class TrackCollection>
void MuonExtenderWithMTDT<TrackCollection>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<bool>("doMuon", false);
  descriptions.add("trackExtenderWithMTDBase", desc);
}

template <class TrackCollection>
template <class H, class T>
void MuonExtenderWithMTDT<TrackCollection>::fillValueMap(edm::Event& iEvent,
                                                          const H& handle,
...
template <class TrackCollection>
void MuonExtenderWithMTDT<TrackCollection>::produce(edm::Event& ev, const edm::EventSetup& es) {
  ...
  if (!doMuon_)
    ...
    std::cout << "---------------------------- NEW MUON EVENT --------------------------" << std::endl;
  std::cout << "" << std::endl;
  std::cout << "" << std::endl;
  ...
  for (const auto& trjtrk : trjtrks) {
    ...
    LogTrace("MuonExtenderWithMTD") << "MuonExtenderWithMTD: extrapolating track " << itrack
    ...
    if (doMuon_){      
      std::cout << "MuonExtenderWithMTD: extrapolating track " << itrack
		<< " p/pT = " << track->p() << " " << track->pt() << " eta = " << track->eta() << std::endl;
      std::cout << "MuonkExtenderWithMTD: sigma_p = "
		<< sqrt(track->covariance()(0, 0)) * track->p2()
		<< " sigma_p/p = " << sqrt(track->covariance()(0, 0)) * track->p() * 100 << " %" << std::endl;
    }
	...
	if (!trajs.measurements().empty()) {
	  ...
	  std::cout << "" << std::endl;
	  std::cout << "Original outer-TSOS global position: " << tsos.globalPosition() << std::endl;
	  std::cout << "x -> " << tsos.globalPosition().x() << std::endl;
          std::cout << "y -> " << tsos.globalPosition().y() << std::endl;
          std::cout << "z -> " << tsos.globalPosition().z() << std::endl;
          std::cout << "r -> " << tsos.globalPosition().perp() << std::endl;
	  std::cout << "" << std::endl;
	  ...
	  std::cout << "" << std::endl;
	  std::cout << "New outer-TSOS global position in tracker: " << tsos.globalPosition() << std::endl;
	  std::cout << "x -> " << tsos.globalPosition().x() << std::endl;
	  std::cout << "y -> " << tsos.globalPosition().y() << std::endl;
	  std::cout << "z -> " << tsos.globalPosition().z() << std::endl;
	  std::cout << "r -> " << tsos.globalPosition().perp() << std::endl;
      ...
      if (doMuon_) {
	std::cout << "" << std::endl;
	std::cout << "Trajectory State Closest To BeamLine" << std::endl;
	std::cout << "x -> " << tscbl.beamLinePCA().x() << std::endl;
	std::cout << "y -> " << tscbl.beamLinePCA().y() << std::endl;
	std::cout << "z -> " << tscbl.beamLinePCA().z() << std::endl;
	std::cout << "r -> " << tscbl.beamLinePCA().perp() << std::endl;
	std::cout << "" << std::endl;
      }
      ...
      if (tscbl_status) {
        ...
        float pathlength0;
        TrackSegments trs0;
        trackPathLength(trajs, tscbl, prop, pathlength0, trs0, doMuon_);
	//trackPathLength(trajs, tscbl, prop, pathlength0, trs0);

	if (doMuon_)
	  std::cout << "Original track path length: " << pathlength0 << std::endl;
	...
	std::cout << "Failing getTrajectoryStateClosestToBeamLine, no search for hits in MTD!" << std::endl;
      }
#endif
    }    
	...
	std::cout << "" << std::endl;
	std::cout << "InsideOut reconstruction" << std::endl;
	std::cout << "" << std::endl;
	...
	for (auto const& hit : thits) {
	  if (hit->geographicalId().det() == DetId::Tracker){
	    ...
	    std::cout << "muon hit" << std::endl;
	    outerthits.push_back(hit);
	  }else{
	    std::cout << "Where is this hit coming from?" << std::endl;	  
	  }
	}
	thits = innerthits;
	...
	std::cout << "" << std::endl;
	std::cout << "OutsideIn reconstruction" << std::endl;
	std::cout << "" << std::endl;
    ...
    std::cout << "New set of hits created!" << std::endl;
    if (!mtdthits.empty())
      std::cout << "MTD Hits found!!!!" << std::endl;    
    else
      std::cout << "MTD hits not found :(" << std::endl;
    ...
    float pMap = 0.f, betaMap = 0.f, t0Map = 0.f, sigmat0Map = -1.f, pathLengthMap = -1.f, tmtdMap = 0.f,
      sigmatmtdMap = -1.f, tofpiMap = 0.f, tofkMap = 0.f, tofpMap = 0.f, tofmuMap = 0.f, sigmatofpiMap = -1.f, sigmatofkMap = -1.f, sigmatofpMap = -1.f, sigmatofmuMap = -1.f;
    GlobalPoint tmtdPosMap{0., 0., 0.};
    int iMap = -1;
    
    for (const auto& trj : trajwithmtd) {
      const auto& thetrj = (updateTraj_ ? trj : trajs);
      float pathLength = 0.f, tmtd = 0.f, sigmatmtd = -1.f, tofpi = 0.f, tofk = 0.f, tofp = 0.f, tofmu = 0.f, sigmatofpi = -1.f, sigmatofk = -1.f, sigmatofp = -1.f, sigmatofmu = -1.f;
      GlobalPoint tmtdPos{0., 0., 0.};
      LogTrace("MuonExtenderWithMTD") << "MuonExtenderWithMTD: refit track " << itrack << " p/pT = " << track->p()
                                       << " " << track->pt() << " eta = " << track->eta();

      if (doMuon_){
	std::cout << "MuonExtenderWithMTD: refit track " << itrack << " p/pT = " << track->p()
		  << " " << track->pt() << " eta = " << track->eta() << std::endl;
      }else{
	std::cout << "MuonExtenderWithMTD: refit track " << itrack << " p/pT = " << track->p()
                  << " " << track->pt() << " eta = " << track->eta() << std::endl;
      }
	...
	if (doMuon_){
	  std::cout << "MuonExtenderWithMTD: tmtd " << tmtdMap << " +/- " << sigmatmtdMap
		    << " t0 " << t0Map << " +/- " << sigmat0Map << " tof pi/K/p " << tofpiMap
		    << "+/-" << fmt::format("{:0.2g}", sigmatofpiMap) << " ("
		    << fmt::format("{:0.2g}", sigmatofpiMap / tofpiMap * 100) << "%) " << tofkMap
		    << "+/-" << fmt::format("{:0.2g}", sigmatofkMap) << " ("
		    << fmt::format("{:0.2g}", sigmatofkMap / tofkMap * 100) << "%) " << tofpMap
		    << "+/-" << fmt::format("{:0.2g}", sigmatofpMap) << " ("
		    << fmt::format("{:0.2g}", sigmatofpMap / tofpMap * 100) << "%) " << std::endl;

	  std::cout << "" << std::endl;
	  std::cout << "Muon timing: tof mu " << tofmuMap
                    << "+/-" << fmt::format("{:0.2g}", sigmatofmuMap) << " ("
                    << fmt::format("{:0.2g}", sigmatofmuMap / tofmuMap * 100) << "%) " << std::endl;
  ...
  fillValueMap(ev, tracksH, tofmuOrigTrkRaw, tofmuOrigTrkToken_);
  ...
  fillValueMap(ev, tracksH, sigmatofmuOrigTrkRaw, sigmatofmuOrigTrkToken_);
...
template <class TrackCollection>
TransientTrackingRecHit::ConstRecHitContainer MuonExtenderWithMTDT<TrackCollection>::tryBTLLayers(
    const TrajectoryStateOnSurface& tsos,
    const Trajectory& traj,
    const float pmag2,
    const float pathlength0,
  ...
  for (const DetLayer* ilay : layers) {
    LogTrace("MuonExtenderWithMTD") << "Hit search: BTL layer at R= "
                                     << static_cast<const BarrelDetLayer*>(ilay)->specificSurface().radius();

    if (doMuon){
      std::cout << "Hit search: BTL layer at R= "
		<< static_cast<const BarrelDetLayer*>(ilay)->specificSurface().radius() << std::endl;
    }

    fillMatchingHits(ilay, tsos, traj, pmag2, pathlength0, trs0, hits, prop, bs, vtxTime, matchVertex, output, bestHit, doMuon);
  }
...
template <class TrackCollection>
TransientTrackingRecHit::ConstRecHitContainer MuonExtenderWithMTDT<TrackCollection>::tryETLLayers(
    const TrajectoryStateOnSurface& tsos,
    const Trajectory& traj,
    const float pmag2,
    const float pathlength0,
  ...
  for (const DetLayer* ilay : layers) {
    ...
    LogTrace("MuonExtenderWithMTD") << "Hit search: ETL disk at Z = " << diskZ;

    if (doMuon){
      std::cout	<< "Hit search: ETL disk at Z = " << diskZ << std::endl;
    }
...
template <class TrackCollection>
void MuonExtenderWithMTDT<TrackCollection>::fillMatchingHits(const DetLayer* ilay,
                                                              const TrajectoryStateOnSurface& tsos,
                                                              const Trajectory& traj,
                                                              const float pmag2,
                                                              const float pathlength0,
  ...
  if (doMuon && hitMatched){
    std::cout << "MuonExtenderWithMTD: matched hit with time: " << bestHit.hit->time()
	      << " +/- " << bestHit.hit->timeError() << std::endl;
  }
  if (doMuon && !hitMatched){
    std::cout << "WARNING! MuonExtenderWithMTD: no matched hit" << std::endl;
  }
...
template <class TrackCollection>
reco::Track MuonExtenderWithMTDT<TrackCollection>::buildTrack(const reco::TrackRef& orig,
                                                               const Trajectory& traj,
  ...
  std::cout << "start to build track" << std::endl;
  ...
  if (hasMTD) {
    float pathlength;
    TrackSegments trs;
    bool validpropagation = trackPathLength(trajWithMtd, bs, thePropagator, pathlength, trs, doMuon);
    float thit = 0.f;
    float thiterror = -1.f;
    ...
    if (doMuon) {
      std::cout << "MuonExtenderWithMTD: selected #hits " << ihitcount << " from ETL "
		<< ietlcount << std::endl;

      std::cout << "New pathLength with MTD = " << pathlength << std::endl;
    }
      
    auto ihit1 = trajWithMtd.measurements().cbegin(); // Assumes the last measurement is the MTD Hit, not true for muons
	...
	if (doMuon)
	  std::cout << "<----------- COMPUTE TIME OF FLIGHT" << std::endl;
	
        TrackTofPidInfo tofInfo = computeTrackTofPidInfo(
							 lastStep.second, etlpathlength, trs, mtdhit1->time(), mtdhit1->timeError(), 0.f, 0.f, true, TofCalc::kCost, SigmaTofCalc::kCost, doMuon);
        //
	  ...
	  if (doMuon) {
	    std::cout << "MTD tracking hits with zero time uncertainty: " << err1 << " " << err2 << std::endl;
	  }
        } else {
      ...
      std::cout << "MTD hits #" << ihitcount << "ETL hits #" << ietlcount << " anomalous pattern, skipping..." << std::endl;
    }
	...
	std::cout << "<----------- COMPUTE TIME OF FLIGHT" << std::endl;
      TrackTofPidInfo tofInfo = computeTrackTofPidInfo(
						       p.mag2(), pathlength, trs, thit, thiterror, 0.f, 0.f, true, TofCalc::kSegm, SigmaTofCalc::kCost, doMuon);
  ...
  std::cout << "finished to build track" << std::endl;
  
  return routput();
}

template <class TrackCollection>
reco::TrackExtra MuonExtenderWithMTDT<TrackCollection>::buildTrackExtra(const Trajectory& trajectory) const {
  static const string metname = "MuonExtenderWithMTD";
...
template <class TrackCollection>
string MuonExtenderWithMTDT<TrackCollection>::dumpLayer(const DetLayer* layer) const {
  stringstream output;
...
typedef MuonExtenderWithMTDT<reco::TrackCollection> MuonExtenderWithMTD;
