...
void TrackExtenderWithMTDT<TrackCollection>::produce(edm::Event& ev, const edm::EventSetup& es) {
  ...
  if (doMuon_)
    std::cout << "Start Muon extender with MTD" << std::endl;
  else
    std::cout << "Start Track extender with MTD" << std::endl;
  ...
  std::cout << "Loop on trajectory Size -> " << trjtrks.size() << std::endl;
  std::cout << "Check tracks Size       -> " << tracksH->size() << std::endl;
  ...
  for (const auto& trjtrk : trjtrks) {
    ...
    //else{
    //  std::cout	<< "New trajectory ------" << std::endl;
    //  std::cout	<< "TrackExtenderWithMTD: extrapolating track " << itrack
    //            << " p/pT = " << track->p() << " " << track->pt() << " eta = " << track->eta() << std::endl;
    //}
	...
	if (doMuon_){
	  std::cout << "MuonExtenderWithMTD: tmtd " << tmtdMap << " +/- " << sigmatmtdMap
		    << " t0 " << t0Map << " +/- " << sigmat0Map << " tof pi/K/p " << tofmuMap
	...
	std::cout  << "Error in the MTD track refitting. This should not happen" << std::endl;
      }
    }
  ...
  std::cout << "Push results" << std::endl;
...
void TrackExtenderWithMTDT<TrackCollection>::fillMatchingHits(const DetLayer* ilay,
  ...
  if (hitMatched) {
    ...
    std::cout << "TrackExtenderWithMTD: matched hit with time: " << bestHit.hit->time()
	      << " +/- " << bestHit.hit->timeError() << std::endl;
  } else {
    LogTrace("TrackExtenderWithMTD") << "TrackExtenderWithMTD: no matched hit";
    std::cout << "TrackExtenderWithMTD: no matched hit" << std::endl;
  }
#endif
}
